...
func nthNumber(index: TableRef[int64, seq[int64]], start: seq[int], n: int64): int64 =
    var
        i = 0'i64           # iteration counter
        cur = 0'i64         # current number
        pre = 0'i64         # prev number
    assert n > start.len
    # initialize index
    for sn in start:
        i.inc
...
if isMainModule:
    ...
    for input in inputs:
        var index = newTable[int64, seq[int64]]()
        let ns = nthNumber(index, input, 30_000_000'i64)
